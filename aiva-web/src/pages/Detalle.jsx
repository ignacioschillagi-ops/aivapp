...
const SLUG_MAP = {
  ...
  'Remeras y camisetas': 'remeras',
  'Buzos y sweaters': 'buzos',
  'Camperas': 'camperas',
  'Jeans y pantalones': 'pantalones',
  'Zapatillas y Zapatos': 'zapatillas',
  'Tapados y abrigos': 'tapados',
  'Camisas': 'camisas',
  'Blusas': 'blusas',
  'Vestidos y enteritos': 'vestidos',
  'Shorts': 'shorts',
  'Polleras y faldas': 'faldas',
  'Ropa interior': 'ropainterior',
  'Medias': 'medias',
  'Guantes': 'guantes',
  'Musculosas': 'musculosas',
  'Botas': 'botas',
  'Tacos y stilettos': 'tacos',
  'Trajes y sacos': 'trajes',
}
    ...
    <div style={{
      ...
      {imgOk ? (
        ...
        <div style={{
          ...
          {slug && <span style={{ fontSize: 11, color: 'var(--dark-50)', textAlign: 'center' }}>
            Subí <strong>{slug}.svg</strong> a <code>public/ilustraciones/</code>
          </span>}
        </div>
      )}
      {lineas.length > 0 && (
            ...
            return (
              <g key={i} onClick={() => campo && onClickLinea(campo.id)} style={{ cursor: campo ? 'pointer' : 'default' }}>
                ...
                <line
                  ...
                  stroke={isActive || isFilled ? '#f58e81' : '#aaa'}
                  strokeWidth={isActive ? 8 : isFilled ? 6 : 5}
                  strokeOpacity={isActive ? 1 : isFilled ? 0.9 : 0.5}
                  strokeLinecap="round"
...
export default function Detalle() {
  ...
  // Load campos: first from static config, then from saved custom categories
  const campos = useMemo(() => {
    const base = CONFIG_MEDIDAS[categoriaDecoded]
    if (base && base.length > 0) return base
    const perfilId = getPerfilActivoId()
    const personalizadas = storage.getJSON(`prendas_personalizadas_${perfilId}`) || []
    const custom = personalizadas.find(p => p.categoria === categoriaDecoded)
    return custom?.campos || []
  }, [categoriaDecoded])
...
function NuevaPrenda() {
  ...
  const guardar = () => {
    ...
    const camposFiltrados = campos.filter(c => c.label.trim()).map(c => ({
      id: c.id, label: c.label.trim(), placeholder: 'ej: 00 cm'
    }))
    const nueva = { id: `custom_${Date.now()}`, categoria: nombre.trim(), campos: camposFiltrados }
    storage.setJSON(clave, [...existentes, nueva])
    // Save any values entered during creation
    const valoresGuardados = {}
    campos.filter(c => c.label.trim() && c.valor.trim()).forEach(c => {
      valoresGuardados[c.id] = c.valor
    })
    setMedidas(perfilId, nombre.trim(), valoresGuardados)
    navigate(-1)
  }
